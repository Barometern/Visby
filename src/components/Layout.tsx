...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      {!isHome && (
        <header className="sticky top-0 z-50 border-b border-medieval-stone/20 bg-background/82 backdrop-blur-xl">
          <div className="container flex h-16 items-center justify-between gap-4 px-4">
            <Link to="/" className="group flex items-center gap-3 min-w-0">
              <div className="min-w-0">
                <div className="font-heading text-lg leading-none text-medieval-gold tracking-[0.08em]">
                  Visby Quest
                </div>
              </div>
            </Link>
      ...
      {!isHome && (
        <nav className="fixed inset-x-0 bottom-0 z-50 px-3 pb-3 md:hidden">
          <div className="mx-auto max-w-md rounded-[1.6rem] border border-medieval-stone/20 bg-background/88 p-2 backdrop-blur-2xl">
