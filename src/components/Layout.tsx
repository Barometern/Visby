...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className={isHome ? 'min-h-screen' : 'flex-1 pb-28 md:pb-0'}>
        {children}
      </main>

      {!isHome && (
        <nav className="fixed inset-x-0 bottom-0 z-50 px-3 pb-3 md:hidden">
          <div className="mx-auto max-w-md rounded-[1.75rem] border border-medieval-stone/20 bg-background/90 px-2.5 py-2.5 backdrop-blur-2xl shadow-[0_-8px_28px_rgba(0,0,0,0.14)]">
              ...
              {navItems.map(({ to, icon: Icon, label }) => {
                ...
                return (
                  <Link key={to} to={to} className="flex min-w-0 flex-1 justify-center">
                    <div
                      className={[
                        'flex min-h-[56px] w-full flex-col items-center justify-center rounded-2xl px-3 py-3 transition-all duration-200',
                        active
