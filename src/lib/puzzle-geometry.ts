...
function piecePathParametric(col: number, row: number, cols: number, rows: number): string {
...
export function getPuzzleConfig(total: 10 | 15): PuzzleConfig {
  ...
  const piecePaths = Array.from({ length: total }, (_, i) =>
    piecePathParametric(i % cols, Math.floor(i / cols), cols, rows),
  );
  puzzleConfigCache[total] = { cols, rows, total, piecePaths };
